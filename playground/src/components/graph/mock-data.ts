...
export function generateMockNodes(): GraphNode[] {
  ...
  documentNodes.forEach((doc, idx) => {
    const angle = (idx / documentNodes.length) * Math.PI * 2;
    const distance = 300 + Math.random() * 100; // 300-400 범위로 확대
  ...
  conceptNodes.forEach((concept, idx) => {
    nodes.push({
      id: concept.id!,
      label: concept.label!,
      type: "concept",
...
export function generateMockEdges(nodes: GraphNode[]): GraphEdge[] {
  ...
  // 배경 노드 연결
  const backgroundNodes = nodes.filter((n) => n.id.startsWith("node-"));
  for (let i = 0; i < 80; i++) {
